...
- [Chaining](../../docs/concepts/#langchain-expression-language) a
...
In this guide we will build an application to translate user input from
...
More commonly, we can “chain” the model with this output parser. This
...
We can easily create the chain using the `.pipe` method. The `.pipe`
...
the `systemTemplate` as well as a simpler template for where the put the
text
...
That’s it! In this tutorial we’ve walked through creating our first
...
For more in-depth tutorials, check out out
[Tutorials](../../docs/tutorials) section.

If you have specific questions on how to accomplish particular tasks,
see our [How-To Guides](../../docs/how_to) section.

For reading up on the core concepts of LangChain, we’ve got detailed
[Conceptual Guides](../../docs/concepts)
