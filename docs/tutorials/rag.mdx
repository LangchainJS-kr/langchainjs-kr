...
미가공 데이터에서 답변까지의 가장 일반적인 전체 시퀀스는 다음과 같습니다:
...
`DocumentLoaders`: 소스에서 데이터를 목록 형식으로 로드하는 클래스입니다.
- [Docs](../../docs/concepts#document-loaders): 사용 방법에 대한 상세 문서
- [Integrations](../../docs/integrations/document_loaders/): 40개 이상의 통합이 있습니다.
...
`TextSplitter`: `Document` 목록을 더 작은 청크로 분할하는 객체이고, `DocumentTransformers`의 하위 클래스입니다. 
  - 컨텍스트들의 분할된 각 위치를 원본 문서에 유지하는 `컨텍스트 인식 스플리터`를 살펴보세요:
    - [Markdown 파일](../../docs/how_to/code_splitter/#markdown): 위 예제에서 사용된 마크다운 코드스플리터 사용법
    - [코드 스플리터](../../docs/how_to/code_splitter/): 15개 이상의 언어 지원
    - [인터페이스](https://v02.api.js.langchain.com/classes/langchain_textsplitters.TextSplitter.html): 기본 인터페이스에 대한 API 레퍼런스
  
`DocumentTransformer`: `Document` 목록에 변환을 수행하는 객체입니다.. 
  - [통합](../../docs/integrations/document_transformers) : 3개 이상의 통합 선택가능
  - [인터페이스](https://v02.api.js.langchain.com/classes/langchain_core_documents.BaseDocumentTransformer.html): 기본 인터페이스에 대한 API 레퍼런스
...
- [통합](../../docs/integrations/text_embedding): 30개 이상의 통합 선택 가능
...
- [통합](../../docs/integrations/vectorstores): 40개 이상의 통합 선택 가능
...
`리트리버`: 텍스트 쿼리에 따라 `문서`를 반환하는 객체입니다 
  - [리트리버 문서](../../docs/concepts#retrievers): 인터페이스와 기본 검색 기술에 대한 추가 문서. 일부 검색 기술에는 다음이 포함됩니다: 
  ...
  - [통합](../../docs/integrations/retrievers): 검색 서비스와의 통합
  - [인터페이스](https://v02.api.js.langchain.com/modules/langchain_core_retrievers.html): 기본 인터페이스에 대한 API 레퍼런스
