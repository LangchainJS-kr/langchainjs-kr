...
이것이 채팅봇이 대화형으로 상호작용할 수 있는 기본 아이디어입니다. 그렇다면
...
포맷팅에 도움이 되는 프롬프트를 추가하여 간단한 체인도 만들어 봅시다:
...
이제 매번 실행 가능한 객체에 전달할 `config`를 생성해야 합니다.
...
하지만 최근 10개의 메시지 안에 있는 정보에 대해 물어보면 여전히 기억하는 것을 확인할 수 있습니다.
...
시간이 걸릴 수 있으므로, 사용자 경험을 향상시키기 위해 대부분의 애플리케이션에서는
...
세부 사항을 더 깊이 파고들고 싶다면 다음 항목을 확인해보세요:
